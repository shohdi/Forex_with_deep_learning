...
void openUp(double lots)
  ...
  int OpenRequestGetAction(int i,bool history)
  {
      ...
      {  
         ask = Ask;
         bid = Bid;
         close = iClose(Symbol(),myPeriod,0);
      }
...
void OnTimer()
{
   MqlDateTime dateTime = getDate();
   if(dateTime.sec == 0 && dateTime.min != enterDateTime.min)
