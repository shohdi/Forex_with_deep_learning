...
class ForexMetaEnv(gym.Env):
    ...
    def step(self,action_idx):
        ...
        if self.openTradeDir == 1 and (self.stepIndex - self.startTradeStep) > (400 * 1) and self.stopTrade:
            action_idx = 2
        elif self.openTradeDir == 2 and (self.stepIndex - self.startTradeStep) > (400 * 1) and self.stopTrade:
            action_idx = 1
        '''
        ...
        if self.startTradeStep is None:
            if self.stepIndex > (400 * 1) and self.punishAgent:
                reward = -0.02
                done = True
